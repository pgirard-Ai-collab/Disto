...
const SECTION_NUMS: Record<SectionKey, string> = {
  brand_identity:       '01',
  mission:              '02',
  brand_intention:      '03',
  archetype:            '04',
  value_proposition:    '05',
  positioning:          '06',
  tone_of_voice:        '07',
  personas:             '08',
  key_messages:         '09',
...
const SECTION_I18N: Record<SectionKey, 'identity' | 'mission' | 'intention' | 'archetype' | 'valueProposition' | 'positioning' | 'tone' | 'personas' | 'keyMessages' | 'manifesto' | 'competitiveContext' | 'values' | 'alwaysSay' | 'neverSay'> = {
...
export default function EditorPanel({ clientId, structureId: initialStructureId, brandName, initialSections, initialStatus }: Props) {
  ...
  const tDesc = useTranslations('admin.editor.sectionDesc');
  ...
  const activeSectionName = tSection(SECTION_I18N[activeKey]);
  const activeSectionDesc = tDesc(SECTION_I18N[activeKey]);
        ...
        <div style={{
          ...
          <div style={{ background: C.bone, padding: '36px 40px', maxWidth: 440, width: '100%' }}>
            ...
            <div style={{ fontSize: 14, color: C.muted, lineHeight: 1.55, marginBottom: 28 }}>
              {t.rich('publishConfirmBody', {
                brand: brandName,
                strong: (chunks) => <strong style={{ color: C.black }}>{chunks}</strong>,
              })}
              {unsaved && <span style={{ display: 'block', marginTop: 8, color: C.yellowDark }}>{t('publishConfirmUnsaved')}</span>}
            </div>
            {publishError && <div style={{ color: C.red, fontSize: 13, marginBottom: 12 }}>{publishError}</div>}
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              <Btn variant="ghost" size="sm" onClick={() => setShowPublishDialog(false)}>{tCommon('cancel')}</Btn>
      ...
      <div className="panel-split" style={{ flex: 1, display: 'flex', overflow: 'hidden', marginTop: 20 }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 'clamp(20px,3vw,44px)', background: C.bone }}>
          ...
          <textarea
            ...
            placeholder={t('sectionContentPlaceholder', { name: activeSectionName })}
            style={{
